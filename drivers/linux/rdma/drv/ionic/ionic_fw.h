...
static inline int ionic_v1_send_wqe_max_sge(u8 stride_log2, int spec, bool expdb)
{
...
static inline int ionic_v1_recv_wqe_max_sge(u8 stride_log2, int spec, bool expdb)
{
	struct ionic_v1_wqe *wqe = (void *)0;
	struct ionic_sge *sge = (void *)(1ull << stride_log2);
	int num_sge = 0;
...
struct ionic_v1_admin_crypto_add_sa {
	...
	__u8				esn_en;
	__u8				sn_overflow_en;
	__u8				byte_lifetime_en;
	__u8				time_lifetime_en;
	__u8				rsvd;
	__le32				protocol;
	__le32				spi;
	__le64				time_lifetime_hard_thresh;
	__le64				time_lifetime_soft_thresh;
	__le64				byte_lifetime_hard_thresh;
	__le64				byte_lifetime_soft_thresh;
	__le32				key[8];
	union {
		struct {
			__u8		replay_prot_en;
			__u8		rsvd[3];
			__le32		replay_prot_window_size;
			__le32		salt;
		} esp;
		struct {
			__u8		squash_udp_en;
			__u8		rsvd[3];
			__le32		udp_src_dest_port;
		} psp;
		struct {
			__u8		squash_udp_en;
			__u8		key_rolling_en;
...
struct ionic_v1_admin_crypto_query_sa {
	__u8				key_size_128;
	__u8				auth_only;
	__u8				esn_en;
	__u8				sn_overflow_en;
	__u8				byte_lifetime_en;
	__u8				time_lifetime_en;
	__u8				time_lifetime_soft_alarm;
	__u8				time_lifetime_hard_alarm;
	__le32				protocol;
	__le32				key_crc;
	__le64				lowest_sn;
	__le64				time_lifetime_hard_thresh;
	__le64				time_lifetime_soft_thresh;
	__le64				byte_lifetime_remaining;
	__le64				byte_lifetime_soft_thresh;
	__le64				seq_num;
	union {
	    struct {
		...
		__le32			salt;
		__le32			rsvd1;
		__le64			replay_window_bit_string[2];
	    } esp;
	    struct {
		__u8			squash_udp_mode;
		__u8			rsvd[3];
		__le32			udp_src_port;
	    } psp;
	    struct {
...
struct ionic_v1_admin_crypto_query_caps {
	struct {
		__u8			ipsec_esp;
		__u8			psp;
		__u8			clustersec;
	...
	struct {
		...
		__u8			re_sync;
		__u8			tfc_padding;
		__u8			byte_time_lifetime;
		__u8			rsvd;
		__le32			max_pad_len;
		__le32			max_sa;
		__le32			min_replay_window_size;
		__le32			max_replay_window_size;
		__le64			max_byte_lifetime_bytes;
		__le64			min_time_lifetime_ms;
		__le64			max_time_lifetime_ms;
	} ipsec;
	struct {
		__u8			tunnel_mode;
		__u8			byte_time_lifetime;
	...
	struct {
		__u8			tunnel_mode;
		__u8			auto_key_rolling;
		__u8			byte_time_lifetime;
		__u8			rsvd;
		__le32			max_cs_groups;
		__le32			max_sa;
		__u8			rsvd1[4];
		__le64			max_byte_lifetime_bytes;
		__le64			min_time_lifetime_ms;
...
enum ionic_v1_admin_op {
	...
	IONIC_V1_ADMIN_CRYPTO_QUERY_CAPS,
	IONIC_V1_ADMIN_CRYPTO_ALLOC_INBOUND_SPI,
	IONIC_V1_ADMIN_CRYPTO_DEALLOC_INBOUND_SPI,
	IONIC_V1_ADMIN_CRYPTO_ADD_SA,
	IONIC_V1_ADMIN_CRYPTO_QUERY_SA,
	IONIC_V1_ADMIN_CRYPTO_DELETE_SA,
...
enum ionic_v1_crypto_sa_dir {
	IONIC_V1_CRYPTO_SA_DIR_TX		= 0,
	IONIC_V1_CRYPTO_RX_DIR_RX		= 1,
};

enum ionic_v1_crypto_protocol {
	IONIC_V1_CRYPTO_STD_PSP			= 0,
	IONIC_V1_CRYPTO_CLUSTERSEC_PSP		= 1,
	IONIC_V1_CRYPTO_IPSEC_ESP		= 2,
	IONIC_V1_CRYPTO_PROTOCOL_RSVD		= 3,
};
