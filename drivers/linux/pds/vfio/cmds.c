...
pds_vfio_suspend_wait_device_cmd(struct pds_vfio_pci_device *pds_vfio, u8 type)
{
	...
	time_done = jiffies;
	dev_dbg(dev, "%s: vf%u: Suspend (type %u) comp received in %d msecs\n", __func__,
		pds_vfio->vf_id, type,
		jiffies_to_msecs(time_done - time_start));

	/* Check the results */
	if (time_after_eq(time_done, time_limit)) {
		dev_err(dev, "%s: vf%u: Suspend (type %u) comp timeout\n", __func__,
			type, pds_vfio->vf_id);
		err = -ETIMEDOUT;
	}

	if (err)
		dev_err(dev, "vf%u: Suspend (type %u) status failed: %pe\n",
			type, pds_vfio->vf_id, ERR_PTR(err));
...
int pds_vfio_suspend_device_cmd(struct pds_vfio_pci_device *pds_vfio, u8 type)
{
	...
	dev_dbg(dev, "vf%u: Suspend (type %u) device\n", pds_vfio->vf_id, type);
	...
	err = pds_vfio_client_adminq_cmd(pds_vfio, &cmd, &comp, true);
	if (err) {
		dev_err(dev, "vf%u: Suspend (type %u) failed: %pe\n", pds_vfio->vf_id,
			type, ERR_PTR(err));
		return err;
	}
...
int pds_vfio_resume_device_cmd(struct pds_vfio_pci_device *pds_vfio, u8 type)
{
	...
	dev_dbg(dev, "vf%u: Resume (type %u) device\n", pds_vfio->vf_id, type);
