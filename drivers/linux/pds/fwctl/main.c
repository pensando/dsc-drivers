...
/* See platform/rtos-sw/src/lib/zrpc/include/zrpc/zrpc_server.h
 * We duplicate these here because hosts that this code
 * will be compiled on won't have access to the zrpc source.
 */
#define ZRPC_CMD_ATTR_READ               0x00
#define ZRPC_CMD_ATTR_DEBUG_READ         0x02
#define ZRPC_CMD_ATTR_WRITE              0x04
#define ZRPC_CMD_ATTR_DEBUG_WRITE        0x08
#define ZRPC_CMD_ATTR_SYNC               0x10
...
static struct pds_fwctl_query_data *pdsfc_get_endpoints(struct pdsfc_dev *pdsfc,
							dma_addr_t *pa)
{
	struct pds_fwctl_query_data_endpoint *entries;
	struct device *dev = &pdsfc->fwctl.dev;
	union pds_core_adminq_comp comp = {0};
							 ...
							 dma_addr_t *pa, u32 ep)
{
	...
	*pa = data_pa;
	...
	for (i = 0; i < num_entries; i++) {
		...
		case ZRPC_CMD_ATTR_READ:
		case ZRPC_CMD_ATTR_WRITE:
		case ZRPC_CMD_ATTR_SYNC:
			entries[i].scope = FWCTL_RPC_CONFIGURATION;
			break;
		case ZRPC_CMD_ATTR_DEBUG_READ:
			entries[i].scope = FWCTL_RPC_DEBUG_READ_ONLY;
			break;
		case ZRPC_CMD_ATTR_DEBUG_WRITE:
			  ...
			  void *in, size_t in_len, size_t *out_len)
{
	...
	if (rpc->in.len > 0) {
		...
		if (!in_payload) {
			dev_dbg(dev, "Failed to allocate in_payload\n");
			err = -ENOMEM;
			goto err_out;
		}
