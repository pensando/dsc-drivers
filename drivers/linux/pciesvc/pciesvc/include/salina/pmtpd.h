...
#ifndef __PCIESVC_SALINA_PMTPD_H__
#define __PCIESVC_SALINA_PMTPD_H__
...
#define PMT_CMN_FIELDS                                    \
    ...
    u_int64_t tblid     :1;     /* table id */            \
    u_int64_t type      :3;     /* PMT_TYPE_* */          \
    u_int64_t port      :2;     /* incoming pcie port */  \
    u_int64_t iden      :1;     /* ide enable */          \
    u_int64_t idepass   :1;     /* ide pass */            \
...
typedef struct {
    PMT_CMN_FIELDS;
    u_int64_t addrdw    :62;    /* tlp address */
...
typedef struct {
    ...
    u_int64_t vfbase    :12;    /* vf base for vf id range for entry */
    ...
    u_int64_t addrdw    :38;    /* target resource address */
    u_int64_t aspace    :1;     /* target address space, 1=external (pcie) */
    u_int64_t romsksel  :7;     /* read-only mask selector */
    u_int64_t idechkdis :1;     /* ide trust check dis */
    u_int64_t spare     :10;    /* implemented but unused in hw */
    u_int64_t rsrv      :10;    /* unimplemented bits */
...
typedef struct {
    u_int64_t valid     :1;     /* entry is valid */
    u_int64_t type      :3;     /* matches PMT.type */
    u_int64_t vfbase    :12;    /* vf base for vf ids valid for entry */
    u_int64_t indirect  :1;     /* sw handles tlp */
    u_int64_t notify    :1;     /* notify sw */
    u_int64_t prtb      :13;    /* base  of contiguous prt entries */
    u_int64_t prtc      :13;    /* count of contiguous prt entries */
    ...
    u_int64_t vflimit   :12;    /* vf field upper limit */
...
typedef struct {
    u_int64_t x;                /* tcam x */
    u_int64_t x1 :8;            /* tcam x1 */
    u_int64_t y;                /* tcam y */
    u_int64_t y1 :8;            /* tcam y1 */
    u_int32_t v  :1;            /* 1=entry valid */
...
#endif /* __PCIESVC_SALINA_PMTPD_H__ */
