...
enum pciehw_cfghnd_e {
    ...
    PCIEHW_CFGHND_DVSEC_INTERNAL_REGS,
};
...
typedef union pciehwbar_u {
    struct {
        ...
        u_int32_t altsize:6;            /* log2 of altsize */
        u_int32_t applyaltsize:1;       /* apply altsize */
...
typedef union pciehwdev_u {
    struct {
        ...
        u_int16_t bm_en:1;              /* busmaster enabled */
        u_int16_t ro_en:1;              /* relaxed order enabled */
        ...
        u_int32_t ro_lif;               /* shadow lif for relaxed order */
    };
    u_int8_t _pad[4096];
} pciehwdev_t;

typedef struct pciehw_indtr_entry_s {
    u_int64_t ts;
    u_int32_t pnd_tm;
    u_int32_t svc_tm;
    u_int32_t err_tm;
} pciehw_indtr_entry_t;

#define PCIEHW_NINDTR           10      /* number of indirect traces */

typedef union pciehw_port_u {
    struct {
        u_int8_t secbus;                /* bridge secondary bus */
        u_int8_t indtimer:1;            /* indirect timer enabled */
        pciemgr_stats_t stats;
        pciehw_indtr_entry_t indtr_ring[PCIEHW_NINDTR];
        u_int8_t indtr_idx;             /* index for next trace */
    };
    u_int8_t _pad[1024];
} pciehw_port_t;
