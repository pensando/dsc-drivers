...
u_int64_t pmt_bar_getaddrmask(const pmt_t *pmt);
...
{
    const u_int64_t data = dm->data.all[0];
    const u_int64_t data1 = dm->data.all[1];
    const u_int64_t mask = dm->mask.all[0];
    const u_int64_t mask1 = dm->mask.all[1];

    pmte->tcam.x =  data & mask;
    pmte->tcam.x1 =  data1 & mask1;
    pmte->tcam.y = ~data & mask;
    pmte->tcam.y1 = ~data1 & mask1;
    pmte->tcam.v = 1;
}
...
{
    dm->data.all[0] = pmte->tcam.x;
    dm->data.all[1] = pmte->tcam.x1;
    dm->mask.all[0] = pmte->tcam.x ^ pmte->tcam.y;
    dm->mask.all[1] = pmte->tcam.x1 ^ pmte->tcam.y1;
}
#else
